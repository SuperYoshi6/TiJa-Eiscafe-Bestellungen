...
const OrderForm: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!supabase) {
      alert('Supabase ist nicht konfiguriert. Bitte verbinden Sie sich mit Supabase.');
      return;
    }
    ...
    if (!drinksOnly && totalScoops === 0) {
      alert('Bitte wählen Sie mindestens eine Kugel Eis oder aktivieren Sie "Nur Getränke"!');
      return;
    }

    if (drinksOnly && formData.drinks.length === 0) {
      alert('Bitte wählen Sie mindestens ein Getränk!');
      return;
    }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full text-center">
            ...
            <p className="text-gray-600 mb-6">
              Vielen Dank für Ihre Bestellung! Wir freuen uns auf Ihren Besuch! 🍦
            </p>
            <button
